...
(function setupOdishaLandVerification() {
    ...
    function buildUI(){
        ...
        document.getElementById("blv-village").addEventListener("change",async e=>{verificationToken=null;window.__BHUMI_LAND_VERIFICATION_TOKEN=null;const calc=document.getElementById("calc-btn");if(calc)calc.disabled=true;if(!e.target.value)return;try{const d=document.getElementById("blv-district").value;await loadPlots(d,e.target.value);setStatus("Village selected. Select Plot No. and click Verify Plot.","wait");}catch(err){setStatus("Could not load plots for village.","bad");}});
