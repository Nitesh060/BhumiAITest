...
(function setupOdishaLandVerification() {
    ...
    async function loadPlots(districtName,villageName){
        const res=await nativeFetch(`${API}/api/odisha/plots/${encodeURIComponent(districtName)}/${encodeURIComponent(villageName)}`);
        const data=await res.json();
        if(!res.ok||!data.plots) throw new Error("Could not load plots from backend");
        const items=data.plots.map(p=>({value:p,label:`Plot ${p}`}));
        setOptions("blv-plot",items,"Select Plot No.");
    }
    ...
    function buildUI(){
        ...
        document.getElementById("blv-village").addEventListener("change",async e=>{verificationToken=null;window.__BHUMI_LAND_VERIFICATION_TOKEN=null;const calc=document.getElementById("calc-btn");if(calc)calc.disabled=true;if(!e.target.value)return;try{const d=document.getElementById("blv-district").value;await loadPlots(d,e.target.value);const c=await villageGeometry(e.target.value);if(c&&cadMap){cadMap.flyTo({center:c,zoom:15});}setStatus("Village selected. Select Plot No. and click Verify Plot.","wait");}catch(err){setStatus("Village selected, but could not load plots or zoom to it.","bad");}});
        document.getElementById("blv-find-plot").addEventListener("click",()=>{const q=document.getElementById("blv-plot").value.trim().toLowerCase();if(!q||!cadMap){setStatus("Select a Plot No. from the dropdown and make sure the cadastral map is loaded.","bad");return;}const layers=parcelLayerIds.filter(id=>id.includes("fill"));const fs=cadMap.queryRenderedFeatures(undefined,{layers});const f=fs.find(x=>(extractPlotNo(x.properties||{})||x.properties?.plot_num?.toString()||"").toLowerCase()===q);if(f){const c=f.geometry?.coordinates?.[0]?.[0]?.[0];const ll=c&&Array.isArray(c)?c:null;selectParcel(f,ll?ll[0]:cadMap.getCenter().lng,ll?ll[1]:cadMap.getCenter().lat);}else setStatus("Plot not found in the currently loaded map view. Zoom into the village and click the parcel.","bad");});
