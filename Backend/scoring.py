...
is intentionally not a score input. Air temperature is accepted only from a
...
def calculate_score(raw_values: Dict[str, Optional[float]], weights: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Adapt API/raw values to the canonical 20-parameter score interface."""
    normalized = dict(raw_values)

    # Accept both the canonical scoring key (air_temp) and the Earth Engine
    # service key (air_temperature). Never overwrite a real canonical value
    # with None. This fixes the previous bug where app.py correctly supplied
    # air_temp but this adapter immediately replaced it with
    # normalized.get("air_temperature") == None.
    if normalized.get("air_temperature") is not None:
    ...
    return {
        ...
        "parameter_groups": comp_result.get("parameter_groups", {}),
        "validation_status": comp_result.get("validation_status"),
    }
